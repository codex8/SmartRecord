...
class Stream(object):
    ...
    def run(self):
        self.cap_Socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # 定义socket类型，网络通信，TCP
        self.cap_Socket.connect((self.IP, self.PORT))
    ...
    def ReadImageStream(self):
        ...
        frameBody = b""
        while True:
            chunk = self.cap_Socket.recv(1024)
            chunk_len = len(chunk)
            if not chunk_len:
                continue
            cursor = 0
